...
void changed_cb(int, int nInserted, int nDeleted, int, const char*, void* v)
{
    ...
    if(loading)
        w->editor->show_insert_position();          //ensure cursor is visible when loading a file
}
...
void load_file(char *newfile, int ipos/*insert position*/)
{
    ...
    if(r)   //if file load/insert failed
        fl_alert("Error loading file '%s': %s",
                     newfile, strerror(errno)); //show alert dialog to user with error message
...
//Function to update the window title based on current filename and modification status
void set_title(Editor_Window *w)
{
    char title[300];
    if(filename[0] == '\0')                         //if file has o name yet
        strcpy(title, "Untitled");                  //set title as "Untitled"
    else                                            //if file has a valid name
        char *slash = strrchr(filename, '/');       //find last '/' in the file path
    #ifdef WIN32                                    //if compiling on Windows
        if(slash == NULL)
            slash = strrchr(filename, '\\');        //on Windows, also check for backslash
    #endif
        if(slash != NULL)                           //if / found in path  
            strcpy(title, slash+1);                 //set title as the substring after the last '/'
        else                                        //if no / found in path 
            strcpy(title, filename);                //set title as the full filename
    
    if(changed)                                     //if file has unsaved changes
        strcat(title, " (modified)");               //append "(modified)" to title
    
    //Finally, set the window title    
    w->label(title); 
}

int main(int argc, char **argv)             //main function - entry point of the application, initializes the application and starts the event loop
{
    textbuf = new Fl_Text_Buffer();             //create a new text buffer to hold the text being edited
    Editor_Window *Window =. new_view();        //create a new editor window instance
    Window->show(1, argv);                      //show the editor window
    if (argc > 1) 
        load_file(argv[1], -1);

return Fl::run();                       //start the FLTK event loop - is infinite, waits for user events & runs until user closes all windows     
}
